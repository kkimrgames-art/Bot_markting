...
def _get_shorts_encoder_settings() -> dict:
    ...
    if is_low_res:
        # 🔧 Render / low-resource: balanced quality (not degraded)
        # CRF 23 + superfast gives far better quality than 28 + ultrafast with minimal overhead
        # min_bitrate 5M ensures YouTube gets enough data for clean re-encode
        settings = {
            "encoder": "libx264",
            "preset": "superfast",
            "crf": "23",
...
class ModVideoProcessor:
    ...
    def _apply_configured_intro_outro_effects(self, input_path: str, output_path: str, video_effects: Optional[Dict[str, Any]]) -> None:
        ...
        if is_low:
            ...
            crf = 14
            ff_threads = 1
            logger.info(f"🔧 [Effects] Low-resource mode (quality): preset={preset}, crf={crf}, threads={ff_threads}")
        else:
    ...
    def _optimize_for_youtube(self, input_path: str, output_path: str) -> bool:
        ...
        try:
            ...
            if rc == 0 and os.path.exists(output_path) and os.path.getsize(output_path) > 0:
                ...
                detail = detail[-400:] if detail else "unknown error"
                logger.warning(f"Optimization failed: {detail}")
                return False
    ...
    def _encode_final_shorts(self, input_path: str, output_path: str, target_fps: Optional[float] = None) -> bool:
        try:
            def _run_encode(enc_settings: dict) -> Tuple[bool, str]:
                ...
                # 🔧 Quality fix: only scale if dimensions are odd; avoid unnecessary resize
                try:
                    vw, vh = self._get_video_dimensions(input_path)
                    needs_scale = (vw % 2 != 0) or (vh % 2 != 0)
                except Exception:
                    needs_scale = True
                vf_str = "scale=trunc(iw/2)*2:trunc(ih/2)*2,format=yuv420p" if needs_scale else "format=yuv420p"
                ...
                bitrate = str(enc_settings.get("min_bitrate") or "5M")
                try:
                    # Parse numeric part for maxrate/bufsize
                    import re
                    num = float(re.sub(r'[^0-9.]', '', bitrate))
                    unit = re.sub(r'[0-9.]', '', bitrate).lower() or "m"
                    maxrate = f"{num * 2:.0f}{unit}"
                    bufsize = f"{num * 3:.0f}{unit}"
                except Exception:
                    maxrate = "10M"
                    bufsize = "15M"
                ...
                if str(enc_settings.get("encoder") or "libx264").lower() == "libx264":
                    cmd += [
                        "-tune", "film",
                        "-b:v", bitrate,
                        "-maxrate", maxrate,
        ...
        try:
            ...
            def _run_filter(filter_complex: str, map_out: bool = True) -> Tuple[bool, str]:
                ...
                if lossless_intermediate:
                    ...
                    # 🔧 Quality fix: use CRF 14 for intermediates to prevent generation loss
                    _, base_preset, _ = self._shorts_x264_settings()
    ...
    def add_watermark_text(self, input_path: str, output_path: str, text: str, seed: Optional[str] = None, custom_font: Optional[str] = None) -> None:
        ...
        try:
            ...
            # 🔧 Quality fix: use CRF 14 for intermediates
            ff_threads, base_preset, _ = self._shorts_x264_settings()
            if _is_low_resource_env():
                base_preset = "superfast"
                base_crf = 14
            else:
                base_crf = int(os.getenv("SHORTS_INTERMEDIATE_CRF", "14") or "14")
            preset = str(os.getenv("SHORTS_INTERMEDIATE_PRESET", base_preset) or base_preset).strip() or base_preset
    ...
    def _shorts_x264_settings(self) -> Tuple[int, str, int]:
        ...
        if _is_low_resource_env():
            # 🔧 On Render / low-resource: balanced quality settings
            # CRF 23 provides noticeably better quality than 28 while remaining stable
            # superfast is significantly better quality than ultrafast with minimal speed loss
            preset = "superfast"
            crf = 23
            ff_threads = 1
    ...
    def _convert_to_shorts(self, input_path: str, output_path: str, orig_width: int, orig_height: int, shorts_format: str = "crop", hflip: bool = False):
        ...
        if lossless_intermediate:
            ...
            # 🔧 Quality fix: use CRF 14 for intermediates (near-lossless) to prevent generation loss
            # This preserves quality through multiple processing stages far better than CRF 23/28
            _, base_preset, _ = self._shorts_x264_settings()
            if is_low:
                # On Render: still use superfast for speed, but CRF 14 for quality
                ...
                x264_crf = int(os.getenv("SHORTS_INTERMEDIATE_CRF", "14") or "14")
            v_profile = "high"
            v_pix_fmt = "yuv420p"
    ...
    def _apply_outro_blur_black(self, input_path: str, output_path: str, duration: float = 1.0):
        ...
        has_audio = self._has_audio(input_path)
        ff_threads, preset, _ = self._shorts_x264_settings()
        level = (os.getenv("SHORTS_H264_LEVEL", "5.1") or "5.1").strip() or "5.1"
        try:
            lossless_intermediate = str(os.getenv("SHORTS_INTERMEDIATE_LOSSLESS", "0") or "0").strip().lower() in {"1", "true", "yes", "on"}
        except Exception:
        ...
        if lossless_intermediate:
            ...
            # 🔧 Quality fix: use CRF 14 for intermediates
            preset = "superfast" if _is_low_resource_env() else preset
            crf = 14 if _is_low_resource_env() else int(os.getenv("SHORTS_INTERMEDIATE_CRF", "14") or "14")
    ...
    def _add_cta_text_reliable(self, input_path: str, output_path: str, text: str, duration: float, custom_font: Optional[str] = None):
        ...
        try:
            ...
            # 🔧 Quality fix: use CRF 14 for intermediates
            ff_threads, base_preset, _ = self._shorts_x264_settings()
            preset = "superfast" if _is_low_resource_env() else str(os.getenv("SHORTS_INTERMEDIATE_PRESET", base_preset) or base_preset).strip() or base_preset
            crf = 14 if _is_low_resource_env() else int(os.getenv("SHORTS_INTERMEDIATE_CRF", "14") or "14")
